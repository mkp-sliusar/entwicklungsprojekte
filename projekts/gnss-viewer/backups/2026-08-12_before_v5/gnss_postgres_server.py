...
<div id="legend">
    ...
    <div class="vehicle-data">
        ...
        <div class="ref-dist">
            ...
            <span class="ref-extra">Extra: <span id="f1-dist-extra">–</span></span>
        </div>
    </div>
    ...
    <div class="vehicle-data">
        ...
        <div class="ref-dist">
            ...
            <span class="ref-extra">Extra: <span id="f2-dist-extra">–</span></span>
        </div>
    </div>

    <div class="ref-tools" title="Extra-Referenz: 51.04471185, 13.727260815">
...
async function loadAxes() {
    ...
    try {
        const body = await fetchJson("/api/axes");
        ...
        if (!state.records.length && axesBounds.isValid()) {
...
function setExtraReference(lat, lon, custom = true, source = "custom") {
    ...
    if (custom) {
        ...
        localStorage.removeItem(EXTRA_REF_STORAGE_KEY);
    }
...
function popupFor(point) {
    return `
        ...
        Extra-Referenz: ${formatNumber(extraReferenceDistanceM(point), 2, "m")}<br>
        Breite: ${point.lat}<br>
        Länge: ${point.lon}
    `;
}
...
function updateLegend(server, point) {
    ...
    document.getElementById(`${prefix}-dist-extra`).textContent =
        formatNumber(extraReferenceDistanceM(point), 2, "m");
}
...
resetRefButton.addEventListener("click", () => {
    setExtraReference(DEFAULT_EXTRA_REF.lat, DEFAULT_EXTRA_REF.lon, false, "fixed");
...
def api_axes():
    try:
        geojson, source = load_axes_geojson()
        return jsonify(
            **geojson,
            source=source,
            configured=bool(geojson["features"]),
            plan_total_length_m=PLAN_AXIS_STATIONS_M[-1][1],
        )
