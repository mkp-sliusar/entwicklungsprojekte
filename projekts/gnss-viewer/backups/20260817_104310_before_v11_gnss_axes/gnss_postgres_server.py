...
def load_project_axes(
    ...
    if mode == "geojson":
        ...
        geometry = None
        if axes.get("stationing") == "axis_centers":
            geometry = stationed_axes_geometry(geojson)

        return geojson, str(axes.get("source_label", "geojson")), geometry
...
def stationed_axes_geometry(geojson: dict[str, Any]) -> dict[str, Any]:
    """Build an exact centerline station model from surveyed transverse axes."""
    points: list[list[float]] = []

    for feature in geojson.get("features", []):
        properties = feature.get("properties") or {}
        coordinates = (feature.get("geometry") or {}).get("coordinates") or []
        station = properties.get("station_m")

        if station is None or len(coordinates) < 2:
            continue

        first_lon, first_lat = coordinates[0][:2]
        last_lon, last_lat = coordinates[-1][:2]
        points.append(
            [
                (float(first_lat) + float(last_lat)) / 2.0,
                (float(first_lon) + float(last_lon)) / 2.0,
                float(station),
            ]
        )
    ...
    if len(points) < 2:
        raise ValueError("Stationierte Achsen benötigen mindestens zwei Stationen.")
    if any(points[index][2] <= points[index - 1][2] for index in range(1, len(points))):
        raise ValueError("Achsensstationen müssen streng aufsteigend sein.")

    return {
        "kind": "stationed_polyline",
        "points": points,
        "start_station_m": points[0][2],
        "end_station_m": points[-1][2],
    }
...
L.control.layers(
    {
        ...
        "Topografie": topoLayer
    },
    {
        "Ortsnamen auf Satellit": satelliteLabelsLayer
    },
    {
        collapsed: true,
        position: "topright"
...
async function loadAxes(forceFit = false) {
    ...
    try {
        ...
        for (const feature of features) {
            ...
            const displayedStation = properties.plan_station_m ?? properties.station_m;
            const stationText = Number.isFinite(Number(displayedStation))
                ? `<br>Station laut Plan: ${Number(displayedStation).toFixed(0)} m`
                : "";
...
function extraReferenceDistanceM(point) {
    ...
    if (
        state.activeProject?.calculations?.extra === "axis_station_nearby" &&
        state.axisGeometry?.kind === "stationed_polyline"
    ) {
        const directDistanceM = haversineM(
            point.lat, point.lon,
            state.extraReference.lat, state.extraReference.lon
        );
        const maxStationDistanceM = Number(
            state.activeProject.calculations.axis_max_reference_distance_m ?? 2000
        );

        if (directDistanceM <= maxStationDistanceM) {
            return axisStationDistanceM(point, state.axisGeometry);
        }
...
function axisStationDistanceM(point, geometry) {
    const stations = geometry.points || [];
    if (stations.length < 2) return NaN;

    const originLat = Number(stations[0][0]);
    const originLon = Number(stations[0][1]);
    const referenceLatRad = originLat * Math.PI / 180;
    const toLocal = (lat, lon) => ({
        x: (Number(lon) - originLon) * Math.PI / 180 *
            EARTH_RADIUS_M * Math.cos(referenceLatRad),
        y: (Number(lat) - originLat) * Math.PI / 180 * EARTH_RADIUS_M
    });
    const measured = toLocal(point.lat, point.lon);
    ...
    for (let index = 0; index < stations.length - 1; index += 1) {
        ...
        const a = toLocal(first[0], first[1]);
        const b = toLocal(second[0], second[1]);
        const dx = b.x - a.x;
        const dy = b.y - a.y;
        ...
        const rawFraction = (
            (measured.x - a.x) * dx + (measured.y - a.y) * dy
        ) / lengthSquared;
        // Extend stationing beyond the two end axes instead of clamping
        // the projection to the finite centerline.
        let fraction;
        if (index === 0 && rawFraction < 0) {
            fraction = rawFraction;
        } else if (index === stations.length - 2 && rawFraction > 1) {
            fraction = rawFraction;
        } else {
            fraction = Math.max(0, Math.min(1, rawFraction));
        }
        const projectedX = a.x + fraction * dx;
        const projectedY = a.y + fraction * dy;
        const distanceSquared =
            (measured.x - projectedX) ** 2 +
            (measured.y - projectedY) ** 2;

        if (distanceSquared < bestDistanceSquared) {
            bestDistanceSquared = distanceSquared;
            bestStation = Number(first[2]) +
                fraction * (Number(second[2]) - Number(first[2]));
        }
    }

    const stationZeroOffsetM = Number(
        state.activeProject?.calculations?.axis_station_zero_offset_m ?? 0
    );
    return bestStation + stationZeroOffsetM;
}
